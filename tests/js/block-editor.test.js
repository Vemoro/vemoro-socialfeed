...
vm.runInNewContext(source, { window: { wp, lifBlockDefaults: { posts: 24, columns: 3 }, lifBlockPalette: [{ name: 'Green sand', slug: 'lif-green-sand', color: '#F3FAF6' }], lifBlockTypography: { fontSizes: [{ name: 'Large', slug: 'large' }], fontFamilies: [{ name: 'GrueneType Neue', slug: 'gruenetypeneue' }] } } });

assert.equal(registration.name, 'local-instagram-feed/feed');
const output = registration.definition.edit({ attributes: {}, setAttributes() {} });
assert.equal(output.type, 'div');
assert.equal(output.props.className, 'lif-block-editor');
assert.equal(output.props['data-block-wrapper'], '1');
assert.equal(output.children[0][0].type, 'InspectorControls');
assert.equal(output.children[0][1].props.className, 'lif-block-editor__preview');

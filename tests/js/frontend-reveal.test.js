...
const reveal = { hidden: true, dataset: {}, querySelector(selector) { return '[data-lif-feed-more]' === selector ? button : null; } };
const closeButton = { listeners: {}, addEventListener(name, callback) { this.listeners[name] = callback; } };
const close = { hidden: true, dataset: {}, classList: new ClassList(), querySelector(selector) { return '[data-lif-feed-close-button]' === selector ? closeButton : null; } };
...
const posts = [0, 0, 900, 900].map((offsetTop) => ({ offsetTop, offsetHeight: 900, querySelector(selector) { return '.lif-post__stage' === selector ? stage : null; } }));
const style = {
	values: {}, maxHeight: '',
...
const feed = {
	...
	querySelectorAll(selector) { if ('.lif-post' === selector) { return posts; } if ('[data-lif-video]' === selector) { return videos; } return []; },
	querySelector(selector) { if ('[data-lif-feed-reveal]' === selector) { return reveal; } if ('[data-lif-feed-close]' === selector) { return close; } return null; },
	getBoundingClientRect() { return { height: '1' === this.dataset.lifExpanded ? 3100 : 1100, top: 0, bottom: 3100 }; },
...
assert.equal(style.values['--lif-collapsed-height'], '1100px');
assert.equal(reveal.hidden, false);
button.listeners.click();
assert.equal(feed.dataset.lifExpanded, '1');
...
assert.equal(style.values['--lif-collapsed-height'], undefined);

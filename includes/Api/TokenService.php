...
final class TokenService {
	...
	public function accessToken(): string {
		return (string) ( $this->secrets->get( 'access_token' ) ?? '' ); }
	public function userId(): string {
		$meta = get_option( Config::TOKEN_OPTION, array() );
		return sanitize_text_field( (string) ( $meta['user_id'] ?? '' ) ); }
	public function expiresAt(): int {
		$meta = get_option( Config::TOKEN_OPTION, array() );
		return (int) ( $meta['expires_at'] ?? 0 ); }
	public function isConnected(): bool {
		return '' !== $this->accessToken() && '' !== $this->userId(); }
	public function needsRefresh(): bool {
		return $this->isConnected() && $this->expiresAt() <= time() + ( 7 * DAY_IN_SECONDS ); }
	...
	public function acceptShortLived( array $short ): void {
		...
		if ( ! $this->secrets->store( 'access_token', $long['access_token'] ) ) {
			throw new \RuntimeException( __( 'The access token could not be encrypted.', 'vemoro-socialfeed' ) ); }
		update_option(
			Config::TOKEN_OPTION,
			array(
				'user_id'          => $short['user_id'],
				'expires_at'       => time() + $long['expires_in'],
				'refresh_failures' => 0,
				'last_refresh'     => time(),
			),
			false
	...
	public function acceptLongLived( array $token ): void {
		if ( ! $this->secrets->store( 'access_token', $token['access_token'] ) ) {
			throw new \RuntimeException( __( 'The access token could not be encrypted.', 'vemoro-socialfeed' ) ); }
		update_option(
			Config::TOKEN_OPTION,
			array(
				'user_id'          => $token['user_id'],
				'expires_at'       => time() + max( 3600, $token['expires_in'] ),
				'refresh_failures' => 0,
				'last_refresh'     => time(),
				'provider'         => 'vemoro',
			),
	...
	public function refresh( bool $force = false ): bool {
		if ( ! $this->isConnected() || ( ! $force && ! $this->needsRefresh() ) ) {
			return true; }
		...
		try {
			...
			if ( ! $this->secrets->store( 'access_token', $data['access_token'] ) ) {
				throw new \RuntimeException( 'Token encryption failed.' ); }
			$meta                     = (array) get_option( Config::TOKEN_OPTION, array() );
			$meta['expires_at']       = time() + $data['expires_in'];
			$meta['last_refresh']     = time();
			$meta['refresh_failures'] = 0;
			update_option( Config::TOKEN_OPTION, $meta, false );
			$this->logs->add( 'info', 'Instagram token refreshed.' );
			return true;
		} catch ( \Throwable $e ) {
			$meta                       = (array) get_option( Config::TOKEN_OPTION, array() );
			$failures                   = (int) ( $meta['refresh_failures'] ?? 0 ) + 1;
			$meta['refresh_failures']   = $failures;
			$meta['last_refresh_error'] = sanitize_text_field( $e->getMessage() );
			update_option( Config::TOKEN_OPTION, $meta, false );
			$this->scheduleRetry( $failures );
			$this->logs->add( 'error', 'Instagram token refresh failed.', array( 'error' => $e->getMessage() ) );
			return false;
		}
	}

	public function disconnect(): void {
		$this->secrets->delete( 'access_token' );
		delete_option( Config::TOKEN_OPTION );
		$this->logs->add( 'info', 'Instagram account disconnected.' ); }
	...
	private function getToken( string $url ): array {
		...
		if ( is_wp_error( $response ) ) {
			throw new ApiException( $response->get_error_message(), 0, 0, true ); }
		$status = wp_remote_retrieve_response_code( $response );
		$data   = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( $status < 200 || $status >= 300 || ! is_array( $data ) || empty( $data['access_token'] ) ) {
			throw new ApiException( sanitize_text_field( (string) ( $data['error']['message'] ?? 'Token request failed.' ) ), $status ); }
		return array(
			'access_token' => (string) $data['access_token'],
			'expires_in'   => max( 3600, (int) ( $data['expires_in'] ?? 60 * DAY_IN_SECONDS ) ),
		);
	}

...
final class OAuthService {
	...
	public function authorizationUrl(int $userId): string {
		if (! Config::usesHostedOAuth() && (! Config::appId() || ! Config::appSecret())) { throw new \RuntimeException(__('Configure the Meta App ID and App Secret first.', 'local-instagram-feed')); }
		$state = bin2hex(random_bytes(32));
		set_transient('lif_oauth_state_' . $userId, hash('sha256', $state), self::STATE_TTL);
		if (Config::usesHostedOAuth()) {
			return Config::connectUrl() . '/v1/instagram/authorize?' . http_build_query(array(
	...
	public function exchangeCode(string $code): array {
		if (Config::usesHostedOAuth()) {
			$response = wp_remote_post(Config::connectUrl() . '/v1/instagram/token', array(
				'timeout' => 20,
				'redirection' => 0,
				'headers' => array('Accept' => 'application/json'),
				'body' => array('grant_code' => $code, 'callback_url' => Config::redirectUri()),
			));
			return $this->tokenResponse($response, __('Vemoro could not complete the Instagram connection.', 'local-instagram-feed'));
		}

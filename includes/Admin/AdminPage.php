...
final class AdminPage {
	...
	public function register(): void {
		...
		add_action('admin_post_lif_connect', array($this, 'connect')); add_action('admin_post_lif_disconnect', array($this, 'disconnect')); add_action('admin_post_lif_refresh_token', array($this, 'refresh')); add_action('admin_post_lif_check_connection', array($this, 'check')); add_action('admin_post_lif_cleanup_orphaned_media', array($this, 'cleanupOrphanedMedia')); add_action('admin_post_lif_delete_all_posts', array($this, 'deleteAllPosts'));
	...
	public function menu(): void {
		add_menu_page(__('Vemoro SocialFeed for WP', 'local-instagram-feed'), __('Vemoro SocialFeed', 'local-instagram-feed'), 'manage_options', 'local-instagram-feed', array($this, 'render'), 'dashicons-instagram', 81);
		add_submenu_page('local-instagram-feed', __('Vemoro SocialFeed settings', 'local-instagram-feed'), __('Settings', 'local-instagram-feed'), 'manage_options', 'local-instagram-feed', array($this, 'render'));
	...
	public function render(): void {
		...
		echo '<div class="wrap lif-admin"><div class="lif-brand"><img src="'.esc_url(LIF_PLUGIN_URL.'assets/images/vemoro-logo.svg').'" alt="Vemoro"><h1>' . esc_html__('SocialFeed for WP', 'local-instagram-feed') . '</h1></div><nav class="nav-tab-wrapper">';
	...
	private function connection(): void {
		$meta=(array)get_option(Config::TOKEN_OPTION,array()); $status=(array)get_option(Config::STATUS_OPTION,array()); echo '<div class="lif-card"><h2>'.esc_html__('Connection status','local-instagram-feed').'</h2><p><strong>'.($this->tokens->isConnected()?esc_html__('Connected','local-instagram-feed'):esc_html__('Not connected','local-instagram-feed')).'</strong></p>';
		...
		if(!empty($status['last_error'])){echo '<div class="notice notice-warning inline"><p><strong>'.esc_html__('Last connection or synchronization error:','local-instagram-feed').'</strong> '.esc_html((string)$status['last_error']).' '.esc_html__('The local data was retained. You can reconnect the Instagram account without deleting it.','local-instagram-feed').'</p></div>';}
		echo '<p>'.esc_html__('Redirect URI:','local-instagram-feed').' <code>'.esc_html(Config::redirectUri()).'</code></p><div class="lif-actions">';
		if(!$this->tokens->isConnected()){echo $this->actionButton('lif_connect',__('Connect with Instagram','local-instagram-feed'),'primary');}else{echo $this->actionButton('lif_connect',__('Reconnect with Instagram','local-instagram-feed'),'primary').$this->actionButton('lif_check_connection',__('Check connection','local-instagram-feed')).$this->actionButton('lif_refresh_token',__('Refresh token','local-instagram-feed'));} echo '</div>';
		if($this->tokens->isConnected()){echo '<hr><h3>'.esc_html__('Permanently disconnect','local-instagram-feed').'</h3><p>'.esc_html__('Temporary API or token errors do not delete data. Reconnect the account where possible. If access has been permanently revoked, disconnecting deletes all Instagram Platform Data stored by the plugin. Media created by the plugin are deleted only when they are not referenced elsewhere in WordPress.','local-instagram-feed').'</p><form method="post" action="'.esc_url(admin_url('admin-post.php')).'"><input type="hidden" name="action" value="lif_disconnect">';wp_nonce_field('lif_disconnect');echo '<p><label><input type="checkbox" name="confirm_disconnect" value="1" required> '.esc_html__('I understand that the Instagram connection and all synchronized Platform Data will be permanently deleted.','local-instagram-feed').'</label></p><p><button type="submit" class="button lif-delete-button">'.esc_html__('Disconnect and delete Instagram data','local-instagram-feed').'</button></p></form>';}
		echo '</div>'; $this->credentialsForm();
	}

	private function credentialsForm(): void {
		$s=Config::settings(); echo '<form method="post" action="options.php" class="lif-card"><h2>'.esc_html__('Instagram connection method','local-instagram-feed').'</h2>'; settings_fields('lif_settings_group');
		echo '<p><label><input type="radio" name="'.esc_attr(Config::OPTION).'[oauth_provider]" value="vemoro" '.checked('vemoro',$s['oauth_provider'],false).'> <strong>'.esc_html__('Vemoro Login (recommended)','local-instagram-feed').'</strong></label><br><span class="description">'.esc_html__('No Meta App ID or App Secret is required in WordPress. The login is handled by the Vemoro connection service.','local-instagram-feed').'</span></p>';
		echo '<p><label><input type="radio" name="'.esc_attr(Config::OPTION).'[oauth_provider]" value="custom" '.checked('custom',$s['oauth_provider'],false).'> <strong>'.esc_html__('Own Meta app (expert mode)','local-instagram-feed').'</strong></label><br><span class="description">'.esc_html__('Use your own Meta app and callback configuration.','local-instagram-feed').'</span></p>';
		echo '<h3>'.esc_html__('Expert-mode credentials','local-instagram-feed').'</h3>';
		...
		foreach(Config::settings() as $key=>$value){if(!in_array($key,array('oauth_provider','connect_url','app_id','app_secret','redirect_uri','api_version'),true)){echo '<input type="hidden" name="'.esc_attr(Config::OPTION).'['.esc_attr($key).']" value="'.esc_attr(is_bool($value)?($value?'1':'0'):$value).'">';}}
	...
	private function privacy(): void {
		echo '<div class="lif-card"><h2>'.esc_html__('Privacy by design','local-instagram-feed').'</h2><ul><li>✓ '.esc_html__('External Meta scripts: no','local-instagram-feed').'</li><li>✓ '.esc_html__('Instagram iframes: no','local-instagram-feed').'</li><li>✓ '.esc_html__('Meta CDN images in the frontend: no','local-instagram-feed').'</li><li>✓ '.esc_html__('Browser API calls: no','local-instagram-feed').'</li><li>✓ '.esc_html__('Local media: yes','local-instagram-feed').'</li><li>'.esc_html__('External Instagram links:','local-instagram-feed').' '.(!empty(Config::settings()['show_link'])?esc_html__('enabled','local-instagram-feed'):esc_html__('disabled','local-instagram-feed')).'</li></ul><p>'.esc_html__('Visitors do not connect to Meta when a page is loaded. The website operator’s server communicates with the Instagram API only during OAuth, synchronization and token maintenance.','local-instagram-feed').'</p><p>'.esc_html__('Temporary API or token errors do not trigger deletion. Administrators can reconnect the account. If access is permanently revoked, use “Disconnect and delete Instagram data” to remove all API-derived Platform Data; plugin-owned media referenced elsewhere in WordPress are retained.','local-instagram-feed').'</p><p>'.esc_html__('Add an appropriate description to your privacy policy. This technical information is not legal advice.','local-instagram-feed').'</p></div>';
		$orphans = $this->posts->orphanedOwnedMediaSummary();
		echo '<div class="lif-card"><h2>'.esc_html__('Clean up orphaned media','local-instagram-feed').'</h2><p>'.esc_html__('Finds media created by this plugin that are no longer assigned to the current Instagram data. Files used elsewhere in WordPress are retained and released from plugin ownership.','local-instagram-feed').'</p><p><strong>'.esc_html(sprintf(__('Unassigned plugin media: %1$d (up to %2$s)','local-instagram-feed'),$orphans['candidates'],size_format($orphans['bytes'],2))).'</strong></p>';
		if ($orphans['candidates'] > 0) { echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'"><input type="hidden" name="action" value="lif_cleanup_orphaned_media">'; wp_nonce_field('lif_cleanup_orphaned_media'); echo '<p><label><input type="checkbox" name="confirm_cleanup" value="1" required> '.esc_html__('I understand that safely identified orphaned media files will be permanently deleted.','local-instagram-feed').'</label></p><p><button type="submit" class="button button-secondary">'.esc_html__('Clean up orphaned media','local-instagram-feed').'</button></p></form>'; }
		else { echo '<p>'.esc_html__('No orphaned plugin media were found.','local-instagram-feed').'</p>'; }
		echo '</div>';
	...
	public function connect(): void { $this->guard('lif_connect'); try{$url=$this->oauth->authorizationUrl(get_current_user_id());$allowed=$this->oauth->isHosted()?strtolower((string)wp_parse_url(Config::connectUrl(),PHP_URL_HOST)):'www.instagram.com';if($allowed!==strtolower((string)wp_parse_url($url,PHP_URL_HOST))){throw new \RuntimeException(__('Invalid authorization host.','local-instagram-feed'));}wp_redirect($url,302,'Vemoro SocialFeed for WP');exit;}catch(\Throwable $e){$this->redirectNotice($e->getMessage(),'error');} }
	public function disconnect(): void {
		$this->guard('lif_disconnect');
		if ('1' !== Request::post('confirm_disconnect')) { $this->redirectNotice(__('Disconnection and deletion were not confirmed.','local-instagram-feed'),'error'); }
		$lock = new SyncLock();
		if (! $lock->acquire()) { $this->redirectNotice(__('The connection cannot be removed while a synchronization is running.','local-instagram-feed'),'error'); }
		$type = 'success'; $message = '';
		try {
			$result = $this->posts->deleteAll();
			$this->tokens->disconnect(); delete_option(Config::STATUS_OPTION); delete_transient('lif_sync_progress'); FeedRenderer::clearCache();
			$this->logs->add('info','Instagram connection and Platform Data deleted.',$result);
			$message = sprintf(__('Instagram disconnected. Deleted %1$d posts and %2$d plugin-owned media files; %3$d referenced media files were retained.','local-instagram-feed'),$result['posts'],$result['attachments'],$result['retained_attachments']);
	...
	public function cleanupOrphanedMedia(): void {
		...
		if ('1' !== Request::post('confirm_cleanup')) { $this->redirectNotice(__('Cleanup was not confirmed.','local-instagram-feed'),'error','privacy'); }
		$lock = new SyncLock();
		if (! $lock->acquire()) { $this->redirectNotice(__('Media cannot be cleaned up while a synchronization is running.','local-instagram-feed'),'error','privacy'); }
		$type = 'success'; $message = '';
		try {
			$result = $this->posts->cleanupOrphanedOwnedMedia();
			$this->logs->add('info','Orphaned Instagram media cleanup completed.',$result);
			$message = sprintf(__('Deleted %1$d orphaned media files and freed %2$s. %3$d externally referenced files were retained; %4$d files could not be deleted.','local-instagram-feed'),$result['deleted'],size_format($result['bytes'],2),$result['retained'],$result['failed']);
			if ($result['failed'] > 0) { $type = 'error'; }
		} catch (\Throwable $e) { $message = $e->getMessage(); $type = 'error'; }
		finally { $lock->release(); }

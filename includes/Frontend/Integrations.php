...
final class Integrations {
	...
	public function block(): void {
		...
		$defaults = array('heading'=>'','heading_level'=>2,'heading_font_family'=>'','heading_font_size'=>'','heading_custom_font_size'=>40,'heading_color'=>'','heading_align'=>'','heading_weight'=>'','heading_style'=>'','heading_spacing'=>16,'posts'=>(int)$settings['post_limit'],'columns'=>(int)$settings['columns'],'columns_tablet'=>(int)$settings['columns_tablet'],'columns_mobile'=>(int)$settings['columns_mobile'],'show_caption'=>(bool)$settings['show_caption'],'show_date'=>(bool)$settings['show_date'],'show_username'=>(bool)$settings['show_username'],'show_metrics'=>(bool)$settings['show_metrics'],'show_link'=>(bool)$settings['show_link'],'show_video_indicator'=>true,'caption_length'=>(int)$settings['caption_length'],'aspect_ratio'=>(string)$settings['aspect_ratio'],'order'=>'DESC','class'=>'','section_background'=>'','full_viewport_background'=>false);
	...
	public function render_block(array $attrs): string {
		...
		if ($background) { $extra['style'] = 'background-color:' . $background . ';'; }
	...
	private function block_palette(): array {
		...
		foreach (array('theme', 'default', 'custom') as $origin) {
			foreach ((array) ($settings[$origin] ?? array()) as $color) {
				if (! empty($color['color'])) { $palette[] = $color; }
			}
		}
		$green_sand = '#F3FAF6';
		foreach ($palette as $color) {
			if (strtolower((string) ($color['color'] ?? '')) === strtolower($green_sand)) { return $palette; }
		}
		$palette[] = array('name' => __('Green sand', 'local-instagram-feed'), 'slug' => 'lif-green-sand', 'color' => $green_sand);
		return $palette;
	}
	private function block_typography(): array {
		$result = array('fontSizes' => array(), 'fontFamilies' => array());
		foreach (array('fontSizes', 'fontFamilies') as $type) {
			...
			foreach (array('theme', 'default', 'custom') as $origin) {
				foreach ((array) ($settings[$origin] ?? array()) as $preset) {
					if (! empty($preset['slug']) && ! empty($preset['name'])) { $result[$type][] = array('slug' => $preset['slug'], 'name' => $preset['name']); }
				}
			}
		}
		return $result;
	...
	private function heading_style(array $attrs): string {
		...
		if ('' !== $family) { $styles[] = 'font-family:var(--wp--preset--font-family--' . $family . ')'; }
		if ('custom' === $size) { $styles[] = 'font-size:' . min(120, max(12, (int) ($attrs['heading_custom_font_size'] ?? 40))) . 'px'; }
		elseif ('' !== $size) { $styles[] = 'font-size:var(--wp--preset--font-size--' . $size . ')'; }
		$color = sanitize_hex_color((string) ($attrs['heading_color'] ?? ''));
		if ($color) { $styles[] = 'color:' . $color; }
		$align = (string) ($attrs['heading_align'] ?? '');
		if (in_array($align, array('left', 'center', 'right'), true)) { $styles[] = 'text-align:' . $align; }
		$weight = (string) ($attrs['heading_weight'] ?? '');
		if (in_array($weight, array('400', '700'), true)) { $styles[] = 'font-weight:' . $weight; }
		$font_style = (string) ($attrs['heading_style'] ?? '');
		if (in_array($font_style, array('normal', 'italic'), true)) { $styles[] = 'font-style:' . $font_style; }
		$styles[] = 'margin-bottom:' . min(120, max(0, (int) ($attrs['heading_spacing'] ?? 16))) . 'px';
		return $styles ? ' style="' . esc_attr(implode(';', $styles)) . '"' : '';
	}
	public function detail(): void {
		$slug = sanitize_title((string) get_query_var('lif_detail')); if (! $slug || empty(Config::settings()['local_detail'])) { return; }
		$post = get_page_by_path($slug, OBJECT, Config::POST_TYPE); if (! $post || 'publish' !== $post->post_status) { global $wp_query; $wp_query->set_404(); status_header(404); return; }
		status_header(200); nocache_headers(); $title = esc_html(get_the_title($post));
		...
		wp_head(); echo '</head><body '; body_class('lif-detail-page'); echo '><main class="lif-detail"><h1>' . $title . '</h1>' . $this->renderer->render(array('post_id'=>(int)$post->ID,'posts'=>1,'show_caption'=>true,'show_date'=>true,'show_username'=>true,'detail_context'=>true)) . '</main>'; wp_footer(); echo '</body></html>'; exit;
	}
}

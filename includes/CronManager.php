...
final class CronManager {
	...
	public static function schedule(): void {
		...
		if ( function_exists( 'as_schedule_recurring_action' ) && function_exists( 'as_next_scheduled_action' ) ) {
			if ( ! as_next_scheduled_action( Config::CRON_HOOK, array(), 'local-instagram-feed' ) ) {
				as_schedule_recurring_action( time() + 60, self::seconds( $interval ), Config::CRON_HOOK, array(), 'local-instagram-feed' ); }
		} elseif ( ! wp_next_scheduled( Config::CRON_HOOK ) ) {

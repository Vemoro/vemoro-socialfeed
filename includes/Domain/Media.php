...
final class Media {
	...
	public static function fromArray(array $data): self {
		...
		foreach ((array) ($data['children']['data'] ?? $data['children'] ?? array()) as $child) {
			if (is_array($child)) {
				$children[] = self::fromArray($child + array('caption' => '', 'permalink' => '', 'timestamp' => '', 'username' => ''));
			}
		}
		return new self(
			sanitize_text_field((string) $data['id']), sanitize_textarea_field((string) ($data['caption'] ?? '')),
			strtoupper(sanitize_key((string) $data['media_type'])), strtoupper(sanitize_key((string) ($data['media_product_type'] ?? ''))),
			esc_url_raw((string) ($data['media_url'] ?? '')), esc_url_raw((string) ($data['thumbnail_url'] ?? '')),
			esc_url_raw((string) ($data['permalink'] ?? '')), sanitize_text_field((string) ($data['timestamp'] ?? '')),
			sanitize_text_field((string) ($data['username'] ?? '')), sanitize_text_field((string) ($data['accessibility_caption'] ?? '')), $children,
			max(0, (int) ($data['like_count'] ?? 0)), max(0, (int) ($data['comments_count'] ?? 0))
		);
	}

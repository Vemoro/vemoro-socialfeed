<?php
/**
 * Plugin Name: Local Instagram Feed
 * Description: Synchronisiert Medien eines professionellen Instagram-Kontos serverseitig und gibt sie vollstaendig lokal aus.
 * Version: 1.0.29
	...
	static function (): void {
		if (version_compare(PHP_VERSION, '8.1', '<') || version_compare((string) get_bloginfo('version'), '6.5', '<')) {
			add_action('admin_notices', static function (): void {
				echo '<div class="notice notice-error"><p>' . esc_html__('Local Instagram Feed requires PHP 8.1 and WordPress 6.5 or newer.', 'local-instagram-feed') . '</p></div>';
			});
			return;
		}
